...
export default function App() {
	...
	// Éditer une tâche
	const fn_EditOneTask = (index, updateInput) => {
		const update = taskList.map((currentTask, indexMap) =>
			indexMap == index ? updateInput : currentTask,
		);
		setTaskList(update);
	};

	// Suppression d'une Task selon son index
	const fn_DeleteOneTask = (indexPositionTaskList) => {
							...
							<FlatList
								...
								renderItem={({ item, index }) => (
									<Task
										i={index}
										task={item}
										deleteOneTask={fn_DeleteOneTask}
										editOneTask={fn_EditOneTask}
									/>
								)}
							></FlatList>
						)}
